...
internal static class ServerHost
{
    ...
    private static void OnServerOnClientDisconnected(object? s, ClientEventArgs e) => Console.WriteLine("Client disconnected: {0}:{1}", e.Client.Host, e.Client.Port);

    private static void OnServerOnClientConnected(object? s, ClientEventArgs e) => Console.WriteLine("Client connected: {0}:{1}", e.Client.Host, e.Client.Port);
}